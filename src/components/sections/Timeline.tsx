        ...
        <div className="flex flex-col gap-0 select-none">
          {events.map((event, index) => (
            <motion.div
              ...
              className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-8 items-start group border-t border-primary-900/10 py-10 transition-colors duration-500 hover:bg-primary-900/[0.02]"
            >
              <div className="md:col-span-2 pt-1">
                <Mono className="text-primary-900/40 text-lg md:text-xl group-hover:text-accent transition-colors duration-300 font-semibold">{event.year}</Mono>
              </div>
              <div className="md:col-span-4">
                <h3 className="text-2xl md:text-3xl font-medium tracking-tight mb-2 text-balance text-primary-900 group-hover:translate-x-2 transition-transform duration-500 ease-out">{event.title}</h3>
                <Mono className="text-primary-900/60 font-mono text-[9px] uppercase">{event.company}</Mono>
              </div>
              <div className="md:col-span-6 md:pt-2">
                <Body className="text-primary-800/80 max-w-lg text-sm md:text-base font-light leading-relaxed">{event.description}</Body>
              </div>
            </motion.div>
          ))}
        </div>
